...
`IUserNotificationDeliveryService` desacopla o fluxo de providers externos e identifica explicitamente mensagens de recovery. O default nao entrega nem registra email, URL ou token. `POST /api/multi-family/auth/reset-password` usa `ResetPasswordAsync`, respeita a policy Identity e atualiza o SecurityStamp.
...
- origem configurada impede envenenamento pelo Host header;
