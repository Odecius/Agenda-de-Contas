...
`IPasswordRecoveryDeliveryService` desacopla SMTP, Telegram e outros providers. O provider default nao entrega nem registra email, URL ou token. `POST /api/multi-family/auth/reset-password` usa `ResetPasswordAsync`, respeita a policy Identity e atualiza o SecurityStamp.
